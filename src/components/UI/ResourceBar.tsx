...
export function ResourceBar({ money, lives, score, primaryColor, accentColor, compact }: ResourceBarProps) {
  if (compact) {
    return (
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="hud-panel px-4 py-2.5 flex items-center gap-5"
      >
        {/* Money */}
        <div className="flex items-center gap-2">
          <span className="text-base font-bold" style={{ color: accentColor }}>$</span>
          <motion.span
            key={money}
            initial={{ scale: 1.2 }}
            animate={{ scale: 1 }}
        ...
        <div className="flex items-center gap-2">
          ...
          <motion.span
            ...
            animate={{ scale: 1 }}
            className="text-base font-bold tabular-nums"
            style={{ color: lives <= 5 ? '#ef4444' : '#fff' }}
          >
            {lives}
          </motion.span>
        </div>
